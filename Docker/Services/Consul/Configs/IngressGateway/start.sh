...
case ${NODE_HOST} in
     "node1.vps1.kristianjones.dev" )
           ARGS="-proxy-id node1vps1 -bind-address=node1vps1=0.0.0.0:8443"
           echo "Node1 ${ARGS}"
           ;;
     "node2.vps1.kristianjones.dev" )
           ARGS="-proxy-id node2vps1 -bind-address=node2vps1=0.0.0.0:8443"
           echo "Node2 ${ARGS}"
           ;;
     "node3.vps1.kristianjones.dev" )
           ARGS="-proxy-id node3vps1 -bind-address=node3vps1=0.0.0.0:8443"
           echo "Node3 ${ARGS}"
           ;;
     * )
